...
{
   public List<Item1> inventory = new List<Item1>();
    // 인벤토리를 리스트로 만듭니다.
    private ItemDatabase db;
    // 아이템 데이터베이스는 db로 축약해서 사용합니다.

    public int slotX, slotY;    // 인벤토리 가로 세로 속성 설정 위한 변수
    public List<Item1> slots = new List<Item1>(); // 인벤토리 속성 변수
    ...
    void DrawInventory()
    {
        ...
        for (int j = 0; j < slotY; j++)
        {
            ...
            {
                Rect slotRect = new Rect(i * 52+400, j * 40+70 , 43, 53);
                // 박스 분할하기
